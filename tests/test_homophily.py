...
def test_set_homophily_timestamps():
    ...
    ho.set_homophily_timestamps(timestamps, g)
    assert [0, 1, 1] == [k for i, k in g.nodes.data('feature')]


def test_set_homophily_timestamps_node_list():
    g = nx.Graph();
    g.add_edges_from([(0, 1), (0, 3)])
    ho.set_homophily_timestamps(timestamps, g, node_list=(0, 1, 3))
    assert [0, 1, 1] == [k for i, k in g.nodes.data('feature')]
...
def test_set_homophily_timestamps_use_length():
    g = nx.Graph();
    g.add_edges_from([(0, 1), (0, 2)])
    ho.set_homophily_timestamps(timestamps, g, use_length=True)
